# **WFO LIVING DESIGN DOCUMENT (LDD) & DEVELOPER CONTRACT (v21)**
...
## **PART II: ACTIVE ARCHITECTURE & BASELINE (PROTOTYPE BLUE v0.0.19)**

### **A. ACTIVE BASELINE FLOOR (v21 GROUND TRUTH)**

| Module | Floor Version | Architectural State |
| :--- | :--- | :--- |
| **Prototype Blue** | `v0.0.19` | Active Operational Baseline |
| **`app.js`** | `v0.0.19` | Router, Unified Coordinator, Pairing Modal Driver & Dashboard Mount |
...
| **`ocr.js`** | `v0.0.8` | Tesseract.js Driver, Raw Engine Intake, Boundary Sanitization & Scaled Levenshtein Ceiling |
...
| **`styles.css`** | `v0.0.10` | Global M3 Design System, Pairing Modal & Pill Layouts |
| **`index.html`** | `v0.0.11` | Shell, Declarative Modal Mounts & Pinned Cache Busting |
...
* **Material 3 Controls:** All interactive buttons styled strictly as Android/Material 3 pills (`border-radius: 9999px`) using canonical `.pill-btn` classes (`.btn-primary`, `.btn-secondary`, `.btn-logout`). Ad-hoc inline button styles or non-conforming classes (`.btn-pill`) are strictly prohibited.
* **Top Bar Controls:**
...
* **Modal Standard & Declarative Roots:** All modal mount roots (`#pairing-modal`, `#manifest-modal`, etc.) must be declared statically in `index.html`. Ad-hoc JavaScript string DOM injection for modals is strictly forbidden. Universal dismissal is enforced via `Escape` key (strictly scoped to open modals), backdrop click/tap, or close/cancel buttons. Back-button navigation on mobile is trapped via `history.pushState({ modalId })` and handled via `popstate` to prevent accidental page exits.
...
6. **Code 128 Handheld Barcode Pairing (`js/barcode.js`, `js/app.js` & `index.html`):**
   ...
   * **Declarative M3 Modal Architecture:** The `#pairing-modal` DOM shell is declared statically in `index.html` with canonical Material 3 pill tokens (`.pairing-card`, `.pairing-code-pill`, `.pairing-barcode-box`, `.pill-btn`). JavaScript maintains state idempotently without dynamic HTML string injection.
   * **Universal Modal Trapping:** Opening triggers `history.pushState({ modalId: 'pairing-modal' })`. Mobile hardware back button (`popstate`), backdrop tap, `Escape` key, and Close button cleanly dismiss the modal without tearing down underlying page state.
...
9. **Hardware Sensor Photo Capture & Raw Engine Intake Pipeline (`js/scanner.js` & `js/ocr.js`):**
   ...
   * **Strict Boundary Noise Purging:** Model and color token parsers strictly sanitize leading and trailing non-alphanumeric OCR artifacts (`~`, `"`, `'`, `|`, `:`, `;`, `,`, `-`, `_`, `.`) from both `modelRaw` and `colorRaw` prior to canonical matching.
   * **Tiered Matching & Scaled Levenshtein Ceiling:** Arbitrary prefix substring slicing (`r.substring(0, t.length)`) is permanently removed to prevent false-positive overrides. Matching follows exact match $\rightarrow$ word boundary prefix $\rightarrow$ full string Levenshtein with a strict dynamic ceiling:
     * Short models ($<6$ chars, e.g. `Flip4`): Must match exactly (`dist === 0`).
     * Mid-length models ($6\text{–}10$ chars): Max edit distance $\le 1$.
     * Long models ($>10$ chars): Max edit distance $\le 2$.
     * Edit distances $\ge 3$ are unconditionally rejected.
