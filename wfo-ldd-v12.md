# **WFO LIVING DESIGN DOCUMENT (LDD) & DEVELOPER CONTRACT (v12)**
...
* **Post-Patch Manifest Roster Mandate:** Every patch delivery response (whether single-file or multi-file) MUST conclude with:
  1. Affected Files Notice: `These patches apply to files: [comma-separated paths]`
  2. Sequential Manifest Version Roster: An explicit list of all project modules matching the exact display sequence defined in `MODULE_VERSIONS` (and the on-screen Manifest dialog).
  3. **3-Tier Version Truth Hierarchy:** When reporting module versions in the closing roster:
     * **Tier 1 (Highest / Code Ground Truth):** Literal version in the attached file or `MODULE_VERSIONS` in `app.js`. If modified in the sprint, tagged `[UPDATED]`.
     * **Tier 2 (Runtime Screen Truth):** On-screen manifest screenshot or text snippet provided by PL during the chat session. Tagged `[unchanged - screen sync]`.
     * **Tier 3 (Floor Fallback):** LDD Baseline Floor (see Part II.A). If a module was not attached or shown on screen, the LLM assumes it is at least at the LDD floor version, tagged `[unchanged - LDD floor]`.
...
2. **LocalStorage Staging Architecture & Zero-Quota Defense:**
   * Multi-carrier scans (`tmo`, `vzw`, `att`) stage in local browser storage (`wfo_staged_inventory`).
   * **Storage Footprint Defense:** `localStorage` strictly stores lean parsed JSON row records (<10KB total). Heavy binary thumbnails (`thumb`) and verbose OCR telemetry logs are strictly held in-memory via `sessionMedia` cache, preventing 5MB quota exhaustion defects.
3. **Multi-Page Intake & Intelligent Deduplication:**
   * Each carrier sheet supports up to 5 scanned physical pages (`Page X/5` review badge).
   * Appending a page merges duplicate items matching `(model + capacity + color)` by summing their quantities (`qty += incoming.qty`); distinct items append as new rows.
4. **Single-Batch Commit Pipeline (Rate-Limit Defense):**
   * Staged scans are submitted as **one single atomic REST API commit** (`PUT /contents/stocks.json`) upon tapping "Publish all". Prevents GitHub API rate limits (5,000/hr) and network throttling.
   * Direct commits are gated by an Admin Personal Access Token (PAT) prompted on first publish and stored in `localStorage` (`wfo_admin_pat`). If `stocks.json` is missing from the repository (404), the publish engine auto-seeds the baseline payload without `sha`.
5. **Pricing & BIC Credit Logic:**
   * `null`: Not offered / skipped / ignored (displays `N/A` or `-`).
   * `0`: Free / $0 promo credit.
   * `> 0`: Active monthly dollar promo credit.
6. **Carrier Availability Rules:**
...
* **Module A (Add Inventory / Standardized Report Intake Pipeline - OPERATIONAL BASELINE):**
  ...
  * **Framed Card Viewfinder & Angle Adjuster:** Centered, bounded camera viewport (`8.5:11` US Letter portrait aspect ratio) with corner reticles, file crop preview, and tilt alignment slider.
  * **Immediate Review Transition, Auto-Routing & Multi-Page Append:** Tapping capture or selecting a file immediately routes to Review. The parser automatically detects sheet carrier, routes to the appropriate tab, and supports appending up to 5 physical pages with automatic item quantity merging.
  * **Interactive Review & Inline Card Editing:** Tap-to-edit row model, capacity, color, and quantity, with delete and manual row insertion controls.
  * **Single-Batch GitHub Publish:** One-tap atomic REST API publishing to `stocks.json` gated by Admin PAT dialog.
...
1. **Introductions:** When an LLM first receives this document, it must:
   ...
   * **Optional Manifest Sync:** The PL may optionally provide an on-screen manifest screenshot or text snippet at onboarding to synchronize baseline versions of unattached modules. If omitted, the LLM adopts the LDD Baseline Floor (Part II.A) for unchanged modules in its post-patch roster.
