# **WFO LIVING DESIGN DOCUMENT (LDD) & DEVELOPER CONTRACT (v15)**
...
### **A. ACTIVE BASELINE FLOOR (v15 GROUND TRUTH)**
| Module | Floor Version |
| :--- | :--- |
| **Prototype Blue** | `v0.0.13` |
| **app.js** | `v0.0.13` |
| **auth.js** | `v0.0.2` |
| **scanner.js** | `v0.0.5` |
| **ocr.js** | `v0.0.4` |
...
└── js/
    ...
    └── print.js      (Print document compiler, WYSIWYG overrides, iframe driver)
```
...
8. **Encapsulated Data Ingestion & Crypto Readiness:**
   * All reads of `stocks.json` and `stats.json` must route strictly through isolated helper functions (`fetchStoreInventory()`, `fetchCatalog()`) rather than scattered inline `fetch()` calls.
   * **Architectural Rationale:** Future-proofs upcoming client-side encryption sprints (Web Crypto API / AES-GCM). Decryption routines can be inserted directly into these helper functions without breaking or refactoring consumer UI modules (`print.js`, `staging.js`).
...
* **Module B (Print Inventory Engine) - [COMPLETE / OPERATIONAL BASELINE]:**
  * **2-Column Typewriter Layout:** Fixed-column inline grid (ATT & VZW left, TMO & Apple right) enforcing `table-layout: fixed;` with flexbox strictly banned on table rows.
  * **Apple Segregation Engine:** Apple devices segregated into dedicated table with priority stock resolution: **`ATT` $\rightarrow$ `VZW` $\rightarrow$ `TMO`**.
  * **Legacy Model Vaporizer:** Automatic regex purging of obsolete iPhone 11/12/13 models (`/iPhone\s*(11|12|13)(?!\d)/i`).
  * **Retail Total Pricing Math:** ATT/VZW displays retail total `round(MO * 36)`; TMO displays `$TOTAL ($DP + $MO/mo)`. Missing/null prices format strictly as `"___"`.
  * **WYSIWYG Live Preview Overrides:** In-place cell overrides (tap quantity to prompt, tap item to toggle hide/unhide, right-click to cycle normal $\rightarrow$ partial $\rightarrow$ full highlight). Appends mandatory audit notice `[This document was edited from the original]` upon modification. Pending PL decision, manual edits such as hidden devices, highlights, etc., may be saved across browser sessions, but persistent storage is not yet an implemented feature (currently in-memory only).
  * **Dual Operational Modes:** Instant toggle between "In-Store Inventory" (with `QTY` column) and "Pricing Index" (suppresses `QTY` column and displays pricing reference banner).
  * **Rich-Text Shift Comments:** Live comment line with `<b>`, `<i>`, and `<u>` parsing.
  * **Silent Print Execution & Ghost-Page Defense:** Desktop/WMPC triggers silent background `<iframe>` print with auto-cleanup (5000ms); mobile falls back to `window.print()`. Enforces strict physical 1-page bounds (`max-height: 10.8in !important; overflow: hidden !important;`).
