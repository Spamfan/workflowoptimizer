# **WFO LIVING DESIGN DOCUMENT (LDD) & DEVELOPER CONTRACT (v8)**
...
* **Brevity Mandate:** All LLM responses across all phases (including Sprint Plans, Tech Specs, and clarifications) must be kept strictly brief, concise, and focused purely on essential points wherever possible.
...
* **Modular ES6 SPA Architecture:** Zero monolith. The app lives across lean, decoupled ES6 modules:
    ...
    └── js/
        ...
        ├── scanner.js    (Camera viewfinder, 5:4 crop, 90° rotation, capture)
        ├── ocr.js        (Tesseract worker, Garbage Cleaver, Orphan-Stitcher)
...
* **Module A (Add Inventory / Pluggable Upload Pipeline):**
  * **Core Intake & Parsing Pipeline (Prototype Blue v0.0.4 — Implemented):** Plug-and-play modular implementation (`js/scanner.js`, `js/ocr.js`, `js/staging.js`) featuring 5:4 aspect crop, IRIS anchor-token & orphan-stitcher OCR parsing, and multi-carrier staged card review with inline CRUD editing.
...
1. **Introductions:** When an LLM first receives this document, it must:
   ...
   * **Objective File Verification (Zero-Assumption Rule):** Inspect literal internal metadata of all attached code files (e.g., `<title>` tag, top header comments, or root keys) and verbatim quote each in the onboarding reply regardless of conversational context (e.g., `Attached file inspection: <title>Workflow Optimizer</title> [MATCH]` or `Attached file inspection: <title>Patcher</title> [⚠️ MISMATCH: Expected Workflow Optimizer]`). If a mismatch is detected, explicitly flag it as a prominent warning.
...
This protocol standardizes iterative field testing across new chat branches to prevent token bloat while calibrating OCR:

1. **Trigger `startx` & Ground-Truth Ingestion:**
   * When the PL states `startx`:
     * If an image is attached: The LLM immediately transcribes all visible tabular rows into a clean ground-truth markdown table.
     * If no image is attached: The LLM prompts for the test artifacts (Physical sheet capture, preprocessed canvas image, and raw telemetry report payload).
2. **Telemetry Ingestion & Dynamic Pipeline Trace (Zero Assumptions):**
   * When the PL provides the debug telemetry payload and preprocessed image:
     * **Audit Data Completeness:** Verify all raw lines and scores are present.
     * **Inspect Active Code Sequence:** Inspect the attached code file to determine the literal execution sequence for *this specific build* (e.g., resolution sampled, crop bounds, contrast toner curve order, column threshold formulas). Do not recite dated historical sequences.
     * **Play-by-Play Verification:** Report back what the code did in this run at each stage:
       1. Image Capture / Pan-Crop Geometry.
       2. Contrast / Toner Saturation Adjustments.
       3. Dynamic Column Partitioning & Line Clustering.
       4. Token Extraction (Capacity, Color, Quantity).
       5. Levenshtein Scoring against `stats.json`.
3. **Defect Diagnosis & Immediate Sprint Plan:**
   * Pinpoint the exact failure location (e.g., toner blowout, word-clustering drift, regex omission).
   * Immediately transition to a concise Sprint Plan (SP) proposing targeted code patches for the next minor version bump.